...
class FilaCircular{
    ...
    public FilaCircular(int tamanho){
        serie = new Serie[tamanho+1];
        primeiro = ultimo = cont = 0;
    }
    //------------------------------------------------------------------------------
    //Inserir
    public void inserir(Serie s)throws Exception{
        if(((ultimo + 1)% serie.length) == primeiro)
            remover();

        serie[ultimo] = s.clone();
        ultimo = (ultimo + 1) % serie.length;
        cont++;

        float soma = 0;
        float media;
        for(int i = primeiro;i != ultimo;i = ((i + 1) % serie.length)){
            soma += serie[i].getNumTemp();
        }
        media = soma/cont;
    ...
    public void remover()throws Exception{
        ...
        primeiro = (primeiro + 1) % serie.length;
        cont--;
    }
    //------------------------------------------------------------------------------
    //Mostrar
